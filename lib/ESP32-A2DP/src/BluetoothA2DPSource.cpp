...
void BluetoothA2DPSource::scanSetup(void) {
  if (nvs_init) {
...
void BluetoothA2DPSource::scan(Speakers* speakers) {
  esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, 10, ESP_BT_GAP_DISC_RES_EVT);
  this->speakerList = speakers;
  this->datacom_scanning = true;
}

void BluetoothA2DPSource::stopScan(void) {
  this->datacom_scanning = false;            // make sure not to collide with filter_inquiry_scan_result
  this->datacom_scan_just_finished = true;   // stop ininite loop of ESP_BT_GAP_DISC_STATE_CHANGED_EVT
  esp_bt_gap_cancel_discovery();
}

void BluetoothA2DPSource::shutdown(void) {
...
void BluetoothA2DPSource::bt_app_gap_callback(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param) {
  ...
  if (event == ESP_BT_GAP_DISC_RES_EVT && this->datacom_scanning == true){
    ESP_LOGD(BT_AV_TAG, "%s SUCESSFULLY did custom evt %d", __func__, event);
    
    int    numSpeakers  = this->speakerList->num; // this->speakerList.num should start as 0 per c standards
    bool   found        = false;
    String address      = String(to_str(param->disc_res.bda));
    int    speakerIndex =0;
    for (speakerIndex=0; speakerIndex<numSpeakers; speakerIndex++){
      if (this->speakerList->speakers[speakerIndex*sizeof(Speaker)].address.equals(address)){
        found = true;
        break;
      }
    }
    if (found){
      ...
      this->speakerList->num = this->speakerList->num+1;
      speakerIndex = this->speakerList->num;
      this->speakerList->speakers[speakerIndex*sizeof(Speaker)].address = String(to_str(param->disc_res.bda)); 
    }
    ...
    for (int i = 0; i < param->disc_res.num_prop; i++) {
      ...
      case ESP_BT_GAP_DEV_PROP_RSSI:
        rssi = *(int8_t *)(p->val);
        ESP_LOGI(BT_AV_TAG, "--RSSI: %d", rssi);
        this->speakerList->speakers[speakerIndex*sizeof(Speaker)].RSSI = String(rssi); 
        break;
      case ESP_BT_GAP_DEV_PROP_EIR:
        eir = (uint8_t *)(p->val);
        break;
    ...
    if (eir) {
      ...
      get_name_from_eir(eir, s_peer_bdname, NULL);
      ESP_LOGI(BT_AV_TAG, "--Name: %s", s_peer_bdname);
      this->speakerList->speakers[speakerIndex*sizeof(Speaker)].name = String((char *)s_peer_bdname);; 
    }
    return;
  }
  switch (event) {
  ...
  case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: { //1
    if (this->datacom_scan_just_finished == true || this->shutdownNow==true){
      ESP_LOGI(BT_AV_TAG, "ShutdownNow true");
      this->datacom_scan_just_finished = false;
